...
public class PointArgumentBuilder extends ArgumentBuilder<FabricClientCommandSource, PointArgumentBuilder> {
    ...
    public PointArgumentBuilder(final String literal, TriConsumer<CommandContext<FabricClientCommandSource>, CamPoint, Integer> consumer, CamCommandProcessor processor) {
    ...
    public PointArgumentBuilder(final String literal, BiConsumer<CommandContext<FabricClientCommandSource>, CamPoint> consumer, CamCommandProcessor processor) {
        this.literal = literal;
    ...
    public CommandNode<FabricClientCommandSource> build() {
        ...
        var command = ClientCommands.argument("location", Vec3Argument.vec3()).executes((x) -> {
            Vec3 vec = PosArgHelper.getVec3(x, "location");
            CamPoint point = new CamPoint(vec.x, vec.y, vec.z, 0, 0, 0, 70);
            processPoint(x, point);
            return 0;
        }).then(ClientCommands.argument("rotation", RotationArgument.rotation()).executes(x -> {
            Vec3 vec = PosArgHelper.getVec3(x, "location");
            ...
            processPoint(x, point);
            return 0;
        }).then(ClientCommands.argument("roll", DoubleArgumentType.doubleArg()).executes(x -> {
            Vec3 vec = PosArgHelper.getVec3(x, "location");
            Vec2 rotation = PosArgHelper.getRotation(x, "rotation");
            CamPoint point = new CamPoint(vec.x, vec.y, vec.z, rotation.y, rotation.x, DoubleArgumentType.getDouble(x, "roll"), 70);
            processPoint(x, point);
            return 0;
        }).then(ClientCommands.argument("fov", DoubleArgumentType.doubleArg()).executes(x -> {
            Vec3 vec = PosArgHelper.getVec3(x, "location");
            Vec2 rotation = PosArgHelper.getRotation(x, "rotation");
        ...
        LiteralArgumentBuilder<FabricClientCommandSource> builder = ClientCommands.literal(literal);
        
        if (indexConsumer != null)
            builder = builder.then(RequiredArgumentBuilder.<FabricClientCommandSource, Integer>argument("index", IntegerArgumentType.integer()).executes((x) -> {
