...
public class SceneCommandBuilder {
    ...
    public static void scene(ArgumentBuilder<FabricClientCommandSource, ?> origin, CamCommandProcessor processor) {
        ArgumentBuilder<FabricClientCommandSource, ?> original = origin;
        
        if (processor.requiresSceneName())
            origin = ClientCommands.argument("name", StringArgumentType.string());
        
        origin.then(ClientCommands.literal("clear").executes((x) -> {
        ...
        origin.then(ClientCommands.literal("remove").then(ClientCommands.argument("index", IntegerArgumentType.integer()).executes((x) -> {
        ...
        origin.then(ClientCommands.literal("duration").executes(x -> {
        ...
        origin.then(ClientCommands.literal("loops").executes(x -> {
        ...
        ArgumentBuilder<FabricClientCommandSource, ?> tpO = ClientCommands.literal("goto");
        ArgumentBuilder<FabricClientCommandSource, ?> tp = tpO;
        if (processor.requiresPlayer())
            tp = ClientCommands.argument("players", EntityArgument.players());
        
        tp.then(ClientCommands.argument("index", IntegerArgumentType.integer(0)).executes(x -> {
            int index = IntegerArgumentType.getInteger(x, "index") - 1;
            CamScene scene = processor.getScene(x);
            if (index >= 0 && index < scene.points.size())
        ...
        origin.then(ClientCommands.literal("mode").executes(x -> {
            x.getSource().sendFeedback(Component.translatable("scene.output.mode", processor.getScene(x).mode.title()));
            return 0;
        }).then(ClientCommands.argument("mode", CamModeArgument.mode()).executes(x -> {
        ...
        origin.then(ClientCommands.literal("interpolation").executes((x) -> {
            x.getSource().sendFeedback(Component.translatable("scene.output.interpolation", processor.getScene(x).interpolation.title()));
            return 0;
        }).then(ClientCommands.argument("interpolation", InterpolationArgument.interpolation()).executes((x) -> {
        ...
        origin.then(ClientCommands.literal("smooth_start").executes((x) -> {
            ...
            return 0;
        }).then(ClientCommands.argument("value", BoolArgumentType.bool()).executes((x) -> {
        ...
        origin.then(ClientCommands.literal("spinning_fix").executes((x) -> {
            x.getSource().sendFeedback(Component.translatable("scene.output.pitch_mode", processor.getScene(x).pitchMode.title()));
            return 0;
        }).then(ClientCommands.argument("mode", CamPitchModeArgument.pitchMode()).executes((x) -> {
        ...
        origin.then(ClientCommands.literal("distance_timing").executes((x) -> {
